...
export default function SignupPageClient({ step }: { step?: string }) {
  const router = useRouter()
  const { handleSignup } = useAuth()
  const { showToast } = useToast()
  const stepParam = step ? Number(step) : NaN
  ...
  const handleViewTerms = (currentStep: number) => {
    const safeStep = Math.min(Math.max(currentStep, 1), 3)
