...
export function calculateCategoryDistributionSummary(
  ...
  // Find top category
  const topCategoryData = data.reduce((max, item) => (item.count > max.count ? item : max), {
    range: 'N/A',
    count: 0,
  });
  ...
  const topCategoryPercentage =
    totalDonations > 0 ? Math.round((topCategoryData.count / totalDonations) * 100) : 0;
...
export function calculateDistributionSummary(
  ...
  const mostPopular = data.reduce((max, item) => (item.count > max.count ? item : max), {
    range: 'N/A',
    count: 0,
  });
