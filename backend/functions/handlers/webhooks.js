 ...
 */
const handleAccountUpdatedStripeWebhook = onRequest(
    ...
    async (req, res) => {
      ...
      try {
        ...
        event = stripe.webhooks.constructEvent(
            req.rawBody,
            sig,
            stripeWebhookSecretAccount.value(),
        );
      } catch (err) {
 ...
 */
const handlePaymentCompletedStripeWebhook = onRequest(
    ...
    async (req, res) => {
      ...
      try {
        ...
        event = stripe.webhooks.constructEvent(
            req.rawBody,
            sig,
            stripeWebhookSecretPayment.value(),
        );
      } catch (err) {
        console.error("Webhook Error:", err.message);
        return res.status(400).send(`Webhook Error: ${err.message}`);
